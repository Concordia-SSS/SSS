...
public class ConcordiaAirlineClient {
    ...
    static {
        ...
        CrewImpl tmp = (CrewImpl) ReservationsystemFactoryImpl.init().createPilot();
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (loginUser.getUserType() == RealUserType.USER.value) {
            compFile = "comp_user.txt";
        }
        completors.add(new SimpleCompletor(ConcordiaAirlineClient.class.getResourceAsStream(compFile)));
        ...
        while ((line = reader.readLine("\033[1;33mConcordiaAirline\033[0m> ")) != null) {
            try {
                ...
                if (loginUser.getUserType() == RealUserType.OPERATOR.value) {
                    if (line.trim().equalsIgnoreCase("createGeneralFlight")) {
                        createGeneralFlight();
                    } else if (line.trim().equalsIgnoreCase("listGeneralFlights")) {
                        listGeneralFlight();
                    } else {
                    ...
                    if (line.trim().equalsIgnoreCase("listGeneralFlights")) {
                        listGeneralFlight();
                    } else if (line.trim().equalsIgnoreCase("createBooking")) {
                        processBooking();
                    } else if (line.trim().equalsIgnoreCase("listBookings")) {
                        listBookings();
                    } else if (line.trim().equalsIgnoreCase("listCrew")) {
                        listCrew();
                    } else if (line.trim().equalsIgnoreCase("createCrew")) {
                        createCrew();
                    } else if (line.trim().equalsIgnoreCase("listPassenger")) {
                        listPassenger();
                    } else if (line.trim().equalsIgnoreCase("createPassenger")) {
                        createPassenger();
                    } else {
                        out.println("No such command: \"" + line + "\"");
                        out.flush();
    ...
    private static void printCommandHelp() {
        ...
        out.println("|-----------------------------|");
        out.println("| help - print help page      |");
        out.println("| exit/quit - exit from app   |");
        if (loginUser.getUserType() == RealUserType.OPERATOR.value) {
            out.println("| listGeneralFlights          |");
            out.println("| createGeneralFlights          |");
            out.println("| listCrew                    |");
            out.println("| createCrew                  |");
        }
        out.println("| createBooking               |");
        out.println("| listBookings                |");
        out.println("| listPassenger               |");
        out.println("| createPassenger             |");
    ...
    static {
        ...
        createPassenger("Eugenio", "Wenz", "Toronto", new Date(1972, 12, 4), "Canada");
        createPassenger("Milo", "Deakins", "Montreal", new Date(1982, 4, 1), "Canada");
        createPassenger("Randy", "Taub", "Ottawa", new Date(1979, 2, 9), "Canada");
        createPassenger("Roy", "Boyles", "Montreal", new Date(1978, 3, 5), "Canada");
        /*
         * createPassenger("Brooks", "Platz", "Toronto", new Date(1983, 6, 5), "Canada"); createPassenger("Damian",
         * "Batres", "Montreal", new Date(1982, 8, 8), "Canada"); createPassenger("Dave", "Heras", "Toronto", new
         * Date(1980, 8, 5), "Canada"); createPassenger("Wilfred", "Knupp", "Ottawa", new Date(1972, 1, 10), "Canada");
         */

    }

    private static PassengerImpl createPassenger(String firstName, String familyName, String address, Date birthDate,
            String citizenship) {
        PassengerImpl tmpPassenger;
        tmpPassenger = (PassengerImpl) ReservationsystemFactoryImpl.init().createPassenger();
    ...
    private static void processBooking() {
        BookingImpl tmpBooking = (BookingImpl) ReservationsystemFactoryImpl.init().createBooking();
        // list flights
        ...
        try {
            String tmpReader = reader.readLine("Enter passenger Id to select: ");
            if (tmpReader != null && !tmpReader.equals("")) {

                PassengerImpl p = passengerList.get(tmpReader);
                if (p != null) {
                    tmpBooking.setPassenger(p);
                    out.println("The passenger No. " + tmpReader + " is selected!");
                    out.flush();
                }
            }
        } catch (IOException e) {
